...
CHART_TEMPLATE_PATH := $(CHARTS_DIRECTORY)/templates
...
$(CHARTS_DIRECTORY)/lxcfs-on-kubernetes-$(RELEASE_VERSION).tgz: helm $(CHART_TEMPLATE_PATH)/clusterrole.yaml \
	...
	sed -i.bak -E "s#version: .*#version: $(RELEASE_VERSION)#" ${CHARTS_DIRECTORY}/Chart.yaml
	sed -i.bak -E "s#appVersion: .*#appVersion: $(RELEASE_VERSION)#" ${CHARTS_DIRECTORY}/Chart.yaml
	$(MAKE) $(CHARTS_DIRECTORY)/README.md
	$(HELM) package $(CHARTS_DIRECTORY) \
		--version $(RELEASE_VERSION) \
		--app-version $(RELEASE_VERSION) \
		--destination $(CHARTS_DIRECTORY)
	-mv ${CHARTS_DIRECTORY}/values.yaml.bak ${CHARTS_DIRECTORY}/values.yaml
	-mv ${CHARTS_DIRECTORY}/Chart.yaml.bak ${CHARTS_DIRECTORY}/Chart.yaml
.PHONY: helm-generate
